...
export function createAgentApp(config: Pick<AgentConfig, "CLUSTER_AGENT_API_KEY"> = { CLUSTER_AGENT_API_KEY: "local-agent-key-please-change-32chars" }, provider: AgentProvider = new MockAgentProvider()): FastifyInstance {
  ...
  app.get("/cluster/status", async () => await provider.status());
  app.get("/cluster/nodes", async () => ({ nodes: await provider.nodes() }));
  app.get("/cloud/users", async () => ({ users: await provider.users() }));
  app.post<{ Body: unknown }>("/cloud/users", async (request, reply) => { const body = request.body as Record<string, unknown>; const username = typeof body.username === "string" ? body.username.trim() : ""; const name = typeof body.name === "string" ? body.name.trim() : ""; if (!/^[a-z][a-z0-9-]{2,31}$/.test(username) || name.length < 2 || name.length > 100) return reply.code(400).send({ error: { code: "VALIDATION_ERROR", message: "A valid provider username and display name are required." } }); return reply.code(201).send(await provider.createUser({ username, name })); });
  app.get("/cloud/vms", async () => ({ vms: await provider.vms() }));
  app.get<{ Params: { id: string } }>("/cloud/vms/:id", async (request, reply) => { const vm = await provider.vm(request.params.id); return vm ? vm : reply.code(404).send({ error: { code: "NOT_FOUND", message: "Virtual machine not found." } }); });
  app.post<{ Body: unknown }>("/cloud/vms", async (request, reply) => { const parsed = vmCreateSchema.safeParse(request.body); if (!parsed.success) return reply.code(400).send({ error: { code: "VALIDATION_ERROR", message: parsed.error.message } }); const payload = request.body as Record<string, unknown>; const body = parsed.data; return reply.code(201).send(await provider.createVm({ ...body, ownerId: typeof payload.ownerId === "string" ? payload.ownerId : "agent", ownerName: typeof payload.ownerName === "string" ? payload.ownerName : "Cluster API" })); });
  for (const action of ["start", "stop", "reboot"] as const) app.post<{ Params: { id: string } }>(`/cloud/vms/:id/${action}`, async (request, reply) => { const vm = await provider.vmAction(request.params.id, action); return vm ? vm : reply.code(404).send({ error: { code: "NOT_FOUND", message: "Virtual machine not found." } }); });
  app.delete<{ Params: { id: string } }>("/cloud/vms/:id", async (request, reply) => await provider.deleteVm(request.params.id) ? reply.code(204).send() : reply.code(404).send({ error: { code: "NOT_FOUND", message: "Virtual machine not found." } }));
