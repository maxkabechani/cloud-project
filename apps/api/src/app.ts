...
export async function createApp(config: Config, auth: Pick<Auth, "handler" | "api">, cloud: CloudService, listUsers: UserDirectory) {
  ...
  app.post("/api/vms", async (request, reply) => {
    ...
    try { return reply.code(201).send(await cloud.createVM(user, request.body)); } catch (error) { if (error instanceof Error && error.message === "VALIDATION_ERROR") return reply.code(400).send({ error: { code: "VALIDATION_ERROR", message: "Choose a valid VM name, image, CPU, memory, and disk size." } }); throw error; }
  });
  ...
  app.post("/api/jobs", async (request, reply) => { const user = await requireUser(request, reply); if (!user) return; const body = request.body as { name?: string; program?: string; processCount?: number }; if (!body?.name || !["matrix-multiply", "calculate-pi", "vector-addition"].includes(body.program ?? "") || ![2, 4, 8, 16].includes(body.processCount ?? 0)) return reply.code(400).send({ error: { code: "VALIDATION_ERROR", message: "Choose a predefined MPI program and process count." } }); return reply.code(201).send(await cloud.createJob(user, { name: body.name.trim().slice(0, 60), program: body.program as "matrix-multiply" | "calculate-pi" | "vector-addition", processCount: body.processCount as number })); });
  return app;
}
