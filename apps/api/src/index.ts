...
const app = await createApp(config, createAuth(database.db, config), createCloudService(listUsers, { provider: config.CLOUD_PROVIDER, agentUrl: config.CLUSTER_AGENT_URL, agentApiKey: config.CLUSTER_AGENT_API_KEY }), listUsers);
