...
export function DashboardShell({ section }: { section: Section }) {
  ...
  return <Dashboard01 nav={nav} section={section} sectionLabel={nav.find((item) => item.section === section)?.label ?? "Overview"} user={user} onLogout={logout}><div className="workspace-content"><SectionView section={section} user={user} onToast={(message) => toast.success(message)} /></div></Dashboard01>;
}
